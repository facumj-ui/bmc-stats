...
def parse_match_excel(uploaded_file):
    ...
    for idx, row in df_raw.iterrows():
        ...
        if val0.endswith("F") and "Estadísticas" not in val0 and "CABB" not in val0:
            equipo_actual = val0
            continue

        if val0 != "" and val0 != "Num." and val1 not in ["TOTALES", "Nombre", "nan", ""] and pd.notna(row[1]):
...
def consolidar_estadisticas(df_total):
    ...
    # Redondear todas las columnas numéricas decimales a 2 decimales
...
if uploaded_files:
    ...
    if dfs:
        ...
        with col2:
            busqueda_nombre = st.text_input("Buscar Jugadora por Nombre:", "")
        ...
        st.caption(f"Mostrando **{len(df_filtrado)}** jugadoras de **{len(df_consolidado)}** totales.")

        # Generar formato estricto de 2 decimales en pantalla
        col_config = {}
        ...
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            df_filtrado.to_excel(writer, sheet_name='Mi Equipo 2026', index=False)
        buffer.seek(0)

        st.download_button(
            label="📥 Descargar Excel",
            data=buffer,
            file_name="Stats_MiEquipo_2026.xlsx",
